...
local function destroyHitboxProxy(character)
	local proxy = hitboxProxies[character]
	if not proxy then return end

	hitboxProxies[character] = nil
...
local function restoreHitboxes()
	...
	for character in pairs(hitboxProxies) do
		table.insert(characters, character)
	end

	for _, character in ipairs(characters) do
		destroyHitboxProxy(character)
	end
end
...
local function updateHitboxes()
		...
		if other ~= player then
			...
			if character
				...
				if not proxy or not proxy.Parent then
					...
					weld.Name = "BunduHitboxWeld"
					weld.Part0 = root
					weld.Part1 = proxy
					weld.Parent = proxy

					hitboxProxies[character] = proxy
				else
					proxy.Size = Vector3.new(
						S.hitboxSize,
						S.hitboxSize,
	...
	for character in pairs(hitboxProxies) do
		if not current[character]
			...
			table.insert(stale, character)
		end
	end

	for _, character in ipairs(stale) do
		destroyHitboxProxy(character)
	end
end
...
widgets.hitboxes = addToggle(
	pages.Misc,
	"HITBOX EXPANDER",
	"Transparent proxy • no collision • does not resize player roots",
	function(enabled)
		S.hitboxes = enabled
